...
export default function NewsSlider({ posts, title = 'Latest News' }) {
  // 1. Config:
  // - Removed 'watchDrag: false' -> Manual scrolling is BACK.
  // - stopOnInteraction: false -> Autoplay continues even after you swipe manually.
  const [emblaRef, emblaApi] = useEmblaCarousel(
    { loop: true, align: 'start' },
    [Autoplay({ delay: 12000, stopOnInteraction: false })],
  );

  // 2. Interaction Handler: Buttons kill autoplay temporarily to allow navigation
      ...
      <div className={styles.Viewport} ref={emblaRef}>
        <div className={styles.Container}>
          {posts.map((post, index) => {
            // Handle Sector/Category Labels
            const labels = post.sectors || post.categories || [];
