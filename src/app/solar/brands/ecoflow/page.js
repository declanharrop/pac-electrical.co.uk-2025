...
export async function generateMetadata() {
  ...
  const { data } = await client.query({
    ...
    variables: { id: 'cm87ahamifhnv08mmk3uvfafl' },
  });

  const { page } = data;

  const METADATA = {
    Url: `https://pac-electrical.co.uk/solar/brands/ecoflow`,
...
export default async function EcoFlowPage() {
  const client = getClient();
  const { data } = await client.query({
    query: STANDARD_PAGE_QUERY,
    variables: {
      id: 'cm87ahamifhnv08mmk3uvfafl',
    },
  });
